...
func (a *CodeSmellAgent) Analyze(path string, code string) (Result, error) {
	// This is a basic prompt for detecting code smells.
	// It can be refined later for more specific types of smells.
	prompt := fmt.Sprintf(`Analyze the following %s code for potential code smells (e.g., long functions, duplicated code, complex logic, poor naming).
