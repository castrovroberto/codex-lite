...
func (a *SecurityAgent) Analyze(path string, code string) (Result, error) {
	...
	prompt := fmt.Sprintf(`Review the following %s code for potential security vulnerabilities (e.g., injection flaws, insecure handling of sensitive data, broken access control, weak cryptography).
